...
def footer():
    ...
    # LinkedIn Link: Uses descriptive text with HTML target="_blank"
    linkedin_url = "http://www.linkedin.com/in/arun-sharma-2b87bb384"
    st.markdown(f'<a href="{linkedin_url}" target="_blank">**LinkedIn Profile**</a>', unsafe_allow_html=True)

    # GitHub Link: Uses descriptive text with HTML target="_blank"
    github_url = "https://github.com/as2118181-byte?tab=projects"
    st.markdown(f'<a href="{github_url}" target="_blank">**GitHub Projects**</a>', unsafe_allow_html=True)

    # Email Link: Uses a mailto: link that opens in a new tab
    email_address = "as2118181@gmail.com"
    st.markdown(f'<a href="mailto:{email_address}" target="_blank">📧 Email: {email_address}</a>', unsafe_allow_html=True)
